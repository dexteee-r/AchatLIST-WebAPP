...
const emptyItem = () => ({
  ...
  title: '', url: '', price: '', priority: 'medium', category: '', targetDate: '', notes: '',
  attributes: [], purchased: false, createdAt: Date.now(),
})
...
export default function App(){
  ...
  const categories = useMemo(()=>{
    const s=new Set(); items.forEach(i=>i.category && s.add(i.category));
  ...
  function upsert(e){
    ...
    setItems(prev=> editingId ? prev.map(it=>it.id===editingId?{...it,...draft}:it) : [{...draft},...prev])
    setDraft(emptyItem()); setEditingId(null)
  }
  const edit=(it)=>{setDraft({...it,price:it.price?.toString?.()??it.price}); setEditingId(it.id); window.scrollTo({top:0,behavior:'smooth'})}
  const del=(id)=>{ if(!confirm('Supprimer cet article ?')) return; setItems(prev=>prev.filter(i=>i.id!==id)); if(editingId===id){setEditingId(null);setDraft(emptyItem())}}
  ...
  const updAttr=(idx,field,val)=>setDraft(d=>{const a=[...(d.attributes||[])]; a[idx]={...a[idx],[field]:val}; return {...d,attributes:a}})
  const rmAttr=(idx)=>setDraft(d=>({...d,attributes:d.attributes.filter((_,i)=>i!==idx)}))

  const filtered = useMemo(()=>{
    let out=[...items]
    const q=filters.q.trim().toLowerCase();
    if(q){ out=out.filter(i=>[i.title,i.category,i.notes,i.url,...(i.attributes||[]).map(a=>`${a.key}:${a.value}`)].filter(Boolean).some(t=>t.toLowerCase().includes(q))) }
    if(filters.priority!=='all') out=out.filter(i=>i.priority===filters.priority)
    if(filters.showPurchased!=='all'){ const should=filters.showPurchased==='purchased'; out=out.filter(i=>i.purchased===should) }
    if(filters.category!=='all') out=out.filter(i=>i.category===filters.category)

    out.sort((a,b)=>{ const dir=sort.dir==='asc'?1:-1; switch(sort.by){
    ...
    <div>
      ...
      <main className="container">
        ...
        <section style={{marginTop:16}}>
          {filtered.length===0 ? (
            <div className="panel" style={{textAlign:'center',color:'var(--muted)',padding:'28px'}}>Aucun élément. Ajoute ton premier achat au-dessus 👆</div>
          ) : (
            <ul className="list">
              {filtered.map(it=> (
...
function exportCSV(items){
  const rows=[["title","url","price","priority","category","targetDate","notes","purchased","attributes"],
    ...items.map(i=>[i.title,i.url,i.price,i.priority,i.category,i.targetDate,(i.notes||'').replaceAll('\n',' '),i.purchased?'1':'0',JSON.stringify(i.attributes||[])])]
  const csv=rows.map(r=>r.map(x=>`"${(x??'').toString().replaceAll('"','""')}"`).join(',')).join('\n')
  const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'}); const url=URL.createObjectURL(blob)
  const a=document.createElement('a'); a.href=url; a.download=`liste_achats_${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url)
}
function importJSON(ev,setItems){
  const f=ev.target.files?.[0]; if(!f) return; const r=new FileReader();
  r.onload=()=>{ try{ const p=JSON.parse(r.result); if(!Array.isArray(p)) throw new Error('Format invalide');
    const n=p.map(i=>({ id:i.id||crypto.randomUUID(), title:i.title||'', url:i.url||'', price:i.price??'', priority:PRIORITIES.some(p=>p.id===i.priority)?i.priority:'medium', category:i.category||'', targetDate:i.targetDate||'', notes:i.notes||'', attributes:Array.isArray(i.attributes)?i.attributes:[], purchased:!!i.purchased, createdAt:i.createdAt||Date.now(), }));
    setItems(n)
  }catch(e){ alert('Import échoué: '+e.message)} finally{ev.target.value=''} }
  r.readAsText(f)
}
